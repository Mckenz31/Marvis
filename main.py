...
from custom_tools import CustomGmailToolSpec, CustomGoogleCalendarToolSpec
...
# tools = [ LoadAndSearchToolSpec.from_defaults( tool_list, index_cls = VectorStoreIndex ).to_tool_list() for tool_spec in tools for tool_list in
...
async def chat( message: str = Body(), google_user_file: str = Query(), chat_history: list[str] = Query( [] ) ):
    ...
    response = agent.finalize_response( task.task_id, output )
